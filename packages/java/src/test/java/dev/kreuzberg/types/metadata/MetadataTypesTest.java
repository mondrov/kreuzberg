 ...
 *
 * <p>
 * Tests verify record structure, JSON serialization/deserialization, Jackson
 * annotations, nullability, immutability, and record behavior.
 */
class MetadataTypesTest {
	...
	class RecordStructureTests {
		...
		void testKeywordsIsList() {
			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of("java", "testing", "kreuzberg"), null, null,
					null, null, null, Map.of(), Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
		...
		void testCanonicalUrlRenamed() {
			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of(), null, "https://example.com/canonical", null,
					null, null, Map.of(), Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
		...
		void testOpenGraphIsMap() {
			Map<String, String> ogMap = Map.of("og:title", "Page Title", "og:description", "Page Description",
					"og:image", "https://example.com/image.jpg");

			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of(), null, null, null, null, null, ogMap,
		...
		void testTwitterCardIsMap() {
			Map<String, String> twitterMap = Map.of("twitter:card", "summary_large_image", "twitter:title",
					"Tweet Title", "twitter:description", "Tweet Description");

			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of(), null, null, null, null, null, Map.of(),
					twitterMap, Map.of(), List.of(), List.of(), List.of(), List.of());
	...
	class JsonSerializationTests {
		...
		void testImageMetadataJsonSerialization() throws IOException {
			...
			assertArrayEquals(original.dimensions(), deserialized.dimensions(), "dimensions array should match");
	...
	class RecordBehaviorTests {
		...
		void testRecordInequality() {
			HtmlMetadata metadata1 = new HtmlMetadata("Title1", null, List.of(), null, null, null, null, null, Map.of(),
					Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());

			HtmlMetadata metadata2 = new HtmlMetadata("Title2", null, List.of(), null, null, null, null, null, Map.of(),
					Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
		...
		void testRecordImmutability() {
			...
			HtmlMetadata metadata = new HtmlMetadata(null, null, keywords, null, null, null, null, null, ogMap,
					Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
	...
	class ComplexIntegrationTests {
		...
		void testNestedMetadataStructure() throws IOException {
			List<HeaderMetadata> headers = List.of(new HeaderMetadata(1, "Main Title", "main", 0, 0),
		...
		void testFullMetadataWithStructuredData() throws IOException {
			...
			HtmlMetadata metadata = new HtmlMetadata("Article Title", "Article description", List.of("tech", "news"),
					...
					List.of(), List.of(), structuredData);
		...
		void testMapOrderingPreservation() throws IOException {
			Map<String, String> metaTags = Map.of("viewport", "width=device-width", "charset", "utf-8", "author",
					"John Doe");

			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of(), null, null, null, null, null, Map.of(),
					Map.of(), metaTags, List.of(), List.of(), List.of(), List.of());
		...
		void testLargeKeywordsList() throws IOException {
			List<String> keywords = List.of("keyword1", "keyword2", "keyword3", "keyword4", "keyword5", "keyword6",
					"keyword7", "keyword8", "keyword9", "keyword10");

			HtmlMetadata metadata = new HtmlMetadata(null, null, keywords, null, null, null, null, null, Map.of(),
					Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
	...
	class SpecialCasesTests {
		...
		void testUnicodeCharacters() throws IOException {
			HtmlMetadata metadata = new HtmlMetadata("Unicode Title: 日本語 العربية 🚀", "Description with emoji: 🎉",
					List.of("keyword", "キーワード", "كلمة"), "Author: José García", "https://example.com/ñ", null, null,
					null, Map.of("og:title", "OG: 中文"), Map.of("twitter:card", "summary_large_image"), Map.of(),
		...
		void testVeryLongTextFields() throws IOException {
			...
			HtmlMetadata metadata = new HtmlMetadata(longTitle, longDescription, List.of(), null, null, null, null,
					null, Map.of(), Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
		...
		void testUnmodifiableCollectionsBehavior() {
			HtmlMetadata metadata = new HtmlMetadata(null, null, List.of("k1", "k2"), null, null, null, null, null,
					Map.of("og:k", "v"), Map.of(), Map.of(), List.of(), List.of(), List.of(), List.of());
	...
	class HtmlExtractionIntegrationTests {
		...
		void testExtractHtmlReturnsMetadata() throws IOException, KreuzbergException {
			...
			ExtractionResult result = Kreuzberg.extractBytes(java.nio.file.Files.readAllBytes(testFile), "text/html",
					null);
		...
		void testExtractInvalidInputHandlesGracefully() {
			assertThrows(KreuzbergException.class, () -> Kreuzberg.extractBytes(null, "text/html", null),
					"Should throw exception for null data");

			assertThrows(KreuzbergException.class, () -> Kreuzberg.extractBytes(new byte[0], "text/html", null),
					"Should throw exception for empty data");

			byte[] testData = "<html><body>Test</body></html>".getBytes();
			assertThrows(KreuzbergException.class, () -> Kreuzberg.extractBytes(testData, null, null),
					"Should throw exception for null MIME type");

			assertThrows(KreuzbergException.class, () -> Kreuzberg.extractBytes(testData, "   ", null),
		...
		void testExtractLargeHtmlPerformance() throws IOException, KreuzbergException {
			...
			for (int i = 0; i < 1000; i++) {
				...
				html.append("<a href=\"https://example.com/link").append(i).append("\">Link ").append(i)
						.append("</a>\n");
				html.append("</section>\n");
			}
			...
			ExtractionResult result = Kreuzberg.extractBytes(largeHtmlData, "text/html", null);
			...
			System.out.println("Large HTML extraction took " + duration + "ms for " + largeHtmlData.length + " bytes");
		...
		@Timeout(120)
		@org.junit.jupiter.api.Disabled("Flaky concurrent test that hangs in CI")
		void testExtractConcurrentExecution() throws IOException, InterruptedException, KreuzbergException {
			...
			try {
				...
				boolean allCompleted = executorService.awaitTermination(90, TimeUnit.SECONDS);
				...
				System.out.println("Concurrent extraction: " + successCount.get() + " succeeded, " + errorCount.get()
						+ " failed out of 10 tasks");
		...
		void testExtractHtmlValidatesMetadataStructure() throws IOException, KreuzbergException {
			byte[] testHtml = ("<!DOCTYPE html>" + "<html>" + "<head>" + "<title>Metadata Test</title>"
					+ "<meta name=\"description\" content=\"Description\">"
					+ "<meta name=\"author\" content=\"Author Name\">" + "</head>" + "<body>" + "<h1>Title</h1>"
					+ "<p>Content</p>" + "</body>" + "</html>").getBytes();
		...
		void testExtractHtmlFromBytesWithMimeType() throws KreuzbergException {
			byte[] htmlBytes = ("<html><head><title>Test</title></head>" + "<body><h1>Test Page</h1></body></html>")
					.getBytes();
